...
int main() {
    ...
    // Test round-trip for all key sizes
    std::vector<uint8_t> plaintext = {
        0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff
    };

    // Test AES-128
    std::cout << "=== AES-128 Round-trip ===" << std::endl;
    std::vector<uint8_t> key128 = {
        0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f
    };
    try {
        AESEncryption aes128(key128);
      //  aes128.Set_useAESNI(false); // Force software path for testing
        auto cipher = aes128.encryptECB(plaintext);
        auto decrypted = aes128.decryptECB(cipher);
    ...
    try {
        ...
        auto cipher = aes192.encryptECB(plaintext);
        auto decrypted = aes192.decryptECB(cipher);
    ...
    try {
        ...
        auto cipher = aes256.encryptECB(plaintext);
        auto decrypted = aes256.decryptECB(cipher);
